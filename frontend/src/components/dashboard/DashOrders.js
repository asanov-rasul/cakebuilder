      ...
      <div className={styles.modal}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.modalSection}>
            ...
            {[
              ['Shape', order.cake_shape],
              ['Size', order.cake_size_kg ? `${order.cake_size_kg} kg` : null],
              ['Filling', order.cake_filling],
              ['Cream', order.cake_cream],
              ['Decorations', decorations !== '—' ? decorations : null],
              ['Text on cake', order.cake_text],
            ].filter(([, v]) => v).map(([l, v]) => (
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (prevCountRef.current !== null && newTotal > prevCountRef.current && statusFilter === '') {
        ...
        toast.success(`🔔 ${diff} new order${diff > 1 ? 's' : ''} received!`, { duration: 5000 });
      }
    ...
    <div className={styles.page}>
      ...
      {loading ? (
        ...
        <div className={styles.empty}>
          ...
          <p>{statusFilter ? `No ${STATUS_LABELS[statusFilter]?.toLowerCase()} orders` : 'No orders yet'}</p>
          {!statusFilter && (
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th className={styles.hideSmall}>Delivery</th>
                  <th>Price</th>
                  <th>Status</th>
                  <th></th>
                </tr>
